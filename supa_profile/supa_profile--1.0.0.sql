...
-- ---------------------------------------------------------------------
-- 1. PATTERN CLASSIFIER HELPER
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION @extschema@.classify_values(vals text[])
RETURNS text AS $$
DECLARE
    val text;
    total int := 0;
...
BEGIN
    ...
    -- If 80% or more match a pattern, return it
    ...
    IF use_estimated_stats THEN
        ...
        LOOP
            ...
            IF null_fraction IS NOT NULL THEN
                ...
                max_len := width_stat::bigint; -- fallback estimate
            ELSE
                -- Fallback defaults if ANALYZE hasn't run
            ...
            IF mcv_vals IS NOT NULL AND array_length(mcv_vals, 1) > 0 THEN
                pattern_val := @extschema@.classify_values(mcv_vals);
            ELSE
                pattern_val := @extschema@.infer_pattern(col.data_type);
            END IF;
    ...
    ELSE
        ...
        LOOP
            ...
            IF sample_vals IS NOT NULL AND array_length(sample_vals, 1) > 0 THEN
                pattern_val := @extschema@.classify_values(sample_vals);
            ELSE
                pattern_val := @extschema@.infer_pattern(col.data_type);
            END IF;
